# BUG: merged squares appear in random places, rather than where square2 was located...
...
class Game
  def initialize(window)
      ...
      (0..5).each do |row|
        (0..5).each do |column|
          index = row * 6 + column # finds color_list[index] index value based on column and row coordinates
          @squares.push Square.new(@window, row, column, color_list[index]) # randomly assigns colors to squares
        end
      end
  ...
  # I'm not quite sure how this works in the concrete
  ...
  # I'm not quite sure how this works in the concrete
  def squares_between_in_colum(square1, square2)
    the_squares = []
  ...
  def move(square1, square2)
    ...
    if square1.row == square2.row
      ...
      squares = squares_between_in_colum(square1, square2)
    else
      return
    end
